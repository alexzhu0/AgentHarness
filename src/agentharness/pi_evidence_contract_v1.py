...
_OPAQUE_IDENTIFIER_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9._:@+-]{0,63}\Z")
_SECRET_IDENTIFIER_MARKER_RE = re.compile(
    r"(?:^|[._:@+-])(?:"
    r"sk(?:-proj)?[-_]|gh[porus]_|github_pat_|AKIA|ASIA|eyJ|"
    r"xox[baprs][-_]|AIza|"
    r"(?:bearer|token|secret|password|passwd|api[-_]?key)(?:$|[._:@+-])"
    r")",
    re.IGNORECASE,
)
_HIGH_ENTROPY_IDENTIFIER_RE = re.compile(r"[A-Za-z0-9]{25}")
...
def build_evidence_snapshot_v1(bus_root: str | Path) -> EvidenceSnapshot:
    ...
    root, files = _capture_snapshot_files(Path(bus_root))
    errors: list[str] = []
...
def _capture_snapshot_files(requested_root: Path) -> tuple[Path, dict[str, bytes]]:
    """Capture local bus bytes beneath one stable, no-follow root descriptor."""
    ...
    try:
        ...
        candidates = _enumerate_snapshot_files(root_descriptor)
        declared_total = 0
        for _relative, declared_metadata in candidates:
        ...
        total = 0
        for relative, declared_metadata in candidates:
            declared_size = declared_metadata[3]
            try:
                raw = _bounded_read_at(
                    root_descriptor,
                    relative,
        ...
        if errors:
            raise SnapshotValidationError(errors)
        _verify_snapshot_root_path(requested_root, root_identity)
    finally:
...
def _verify_snapshot_root_path(
    requested_root: Path, expected: tuple[int, int, int]
) -> None:
    try:
        metadata = os.stat(requested_root, follow_symlinks=False)
    except OSError:
        raise SnapshotValidationError(("snapshot.file_changed",)) from None
    if stat.S_ISLNK(metadata.st_mode):
        raise SnapshotValidationError(("snapshot.symlink_not_allowed",))
                ...
                with os.scandir(directory) as iterator:
                    for entry in iterator:
                        ...
                        if stat.S_ISDIR(metadata.st_mode):
                            ...
                            if _directory_identity(os.fstat(child)) != _directory_identity(
                                metadata
                            ):
                                os.close(child)
                                raise SnapshotValidationError(("snapshot.file_changed",))
                        ...
                        candidates.append((relative, _file_identity(metadata)))
...
def _bounded_read_at(
    ...
    """Read one stable regular file relative to a bound root descriptor."""
...
class _UniqueLoader(yaml.SafeLoader):
    def __init__(self, stream) -> None:
        ...
        self._agentharness_compose_depth = 0
        self._agentharness_nodes = 0
    ...
    def compose_node(self, parent, index):
        ...
        self._agentharness_nodes += 1
        if self._agentharness_nodes > MAX_YAML_NODES:
            raise yaml.YAMLError("YAML node budget exceeded")
        self._agentharness_compose_depth += 1
        if self._agentharness_compose_depth > MAX_STRUCTURE_DEPTH:
...
def _unique_mapping(loader, node, deep=False):
    ...
    for key_node, value_node in node.value:
        ...
        if key == "<<":
            raise yaml.YAMLError("YAML merge keys are not allowed")
        if key in result:
...
def _is_opaque_identifier(value: Any) -> bool:
    return (
        ...
        and _OPAQUE_IDENTIFIER_RE.fullmatch(value) is not None
        and _SECRET_IDENTIFIER_MARKER_RE.search(value) is None
        and _HIGH_ENTROPY_IDENTIFIER_RE.search(value) is None
    )
