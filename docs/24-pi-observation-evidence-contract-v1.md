...
- Caller-controlled protocol identifiers are opaque ASCII tokens of at most 64
  characters. URL, path, credential-marker, common provider-token, and long
  high-entropy alphanumeric forms are rejected rather than echoed into evidence
  or reviewer output. This is output minimization, not general secret detection.
