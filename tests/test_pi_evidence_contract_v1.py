...
class PiEvidenceContractV1Tests(unittest.TestCase):
    ...
    def test_disclosure_bearing_call_metadata_rejects_without_echo(self) -> None:
        probes = (
            ...
            ("tool_call_id", "Bearer-secret-token"),
            ("tool_call_id", "xoxb-secret-token"),
            ("tool_name", "AIza-secret-token"),
        )
    ...
    def test_disclosure_bearing_evidence_ids_reject_snapshot_without_echo(self) -> None:
        for field, value in (
            ...
            ("handoff_id", "xoxb-secret-token"),
            ("handoff_id", "AIza-secret-token"),
        ):
    ...
    def test_snapshot_reads_each_file_once(self) -> None:
        ...
        original = contract._bounded_read_at
        counts: dict[str, int] = {}

        def counted(root_descriptor: int, relative: str, limit: int, expected=None) -> bytes:
            counts[relative] = counts.get(relative, 0) + 1
            return original(root_descriptor, relative, limit, expected)

        with patch.object(contract, "_bounded_read_at", counted):
    ...
    def test_snapshot_swap_to_symlink_fails_closed_without_reading_target(self) -> None:
        ...
        with _copy_bus() as bus:
            ...
            def swap(root_descriptor: int, relative: str, limit: int, expected=None) -> bytes:
                nonlocal swapped
                if relative == "ledger.jsonl" and not swapped:
                    victim.unlink()
            ...
            with patch.object(contract, "_bounded_read_at", swap), patch.object(
    ...
    def test_snapshot_root_swap_to_external_symlink_fails_closed(self) -> None:
        from agentharness import pi_evidence_contract_v1 as contract

        with _copy_bus() as bus:
            external_bus = bus.parent / "external-bus"
            original_bus = bus.parent / "original-bus"
            shutil.copytree(bus, external_bus)
            original_is_symlink = Path.is_symlink
            swapped = False

            def swap_after_initial_check(path):
                nonlocal swapped
                result = original_is_symlink(path)
                if path == bus and not swapped:
                    bus.rename(original_bus)
                    bus.symlink_to(external_bus, target_is_directory=True)
                    swapped = True
                return result

            with patch.object(Path, "is_symlink", swap_after_initial_check):
                with self.assertRaises(SnapshotValidationError) as raised:
                    build_evidence_snapshot_v1(bus)

        self.assertTrue(swapped)
        self.assertTrue(
            {"snapshot.symlink_not_allowed", "snapshot.file_changed"}
            & set(raised.exception.codes),
    ...
    def test_snapshot_rejects_yaml_anchors_and_aliases(self) -> None:
        with _copy_bus() as bus:
            (bus / "alias-bomb.yaml").write_text(
                "seed: &seed [value]\nexpanded: [*seed, *seed, *seed, *seed]\n",
                encoding="utf-8",
            )
        ...
        self.assertIn("snapshot.structured_file.malformed", raised.exception.codes)

    def test_snapshot_rejects_yaml_beyond_structure_depth_budget(self) -> None:
        with _copy_bus() as bus:
            depth = MAX_STRUCTURE_DEPTH + 1
            (bus / "deep.yaml").write_text(
                "[" * depth + "leaf" + "]" * depth + "\n",
                encoding="utf-8",
            )
