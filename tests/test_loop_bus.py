...
class LoopBusTests(unittest.TestCase):
    ...
    def test_valid_bus_cli_sanitizes_absolute_path_and_keeps_relative_path(self):
        absolute_stdout = StringIO()
        with redirect_stdout(absolute_stdout):
            absolute_code = main(["loop", "check", str(BUS_ROOT)])

        absolute_output = absolute_stdout.getvalue()
        self.assertEqual(0, absolute_code)
        self.assertIn("PASS loop bus validation:", absolute_output)
        self.assertNotIn(str(BUS_ROOT), absolute_output)
        self.assertNotIn(str(ROOT), absolute_output)

        relative_stdout = StringIO()
        with redirect_stdout(relative_stdout):
            relative_code = main(["loop", "check", "examples/agent_bus"])

        self.assertEqual(0, relative_code)
        self.assertIn("examples/agent_bus", relative_stdout.getvalue())
